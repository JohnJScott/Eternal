...
/*
 * This package is based on the work done by Keiron Liddle, Aftex Software <keiron@aftexsw.com> to whom the Ant project is very grateful for his great code.
 */

//
// Design notes:
//
// This class performs BZip2 compression. It is derived from the BZip2OutputStream from the Apache commons source code, but is
// significantly modified from that code. While the Apache class is a stream that compresses, this particular class simply performs
// compression.  It follows a Manager pattern. It manages an internal buffer for uncompressed data; callers place data into the buffer
// using the Fill() method. This class then compresses the data and writes the compressed form out, via the CompressAndWrite() method.
// Because BZip2 uses byte-shredding, this class relies on a BitWriter, and not a .NET Stream, to emit its output.  (*Think of the BitWriter
// class as an Adapter that enables Bit-oriented output to a standard byte-oriented .NET stream.)
//
// This class exists to support the two distinct output streams that perform BZip2 compression: BZip2OutputStream and
// ParallelBZip2OutputStream. These streams rely on BZip2Compressor to provide the encoder/compression logic.  This code has been derived
// from the bzip2 output stream in the Apache commons library; it has been significantly modified from that form, in order to provide a
// single compressor that could support both types of streams.
//
// In a bz2 file or stream, there is never any bit padding except for 0..7 bits in the final byte in the file. Successive compressed blocks in a
...
namespace Ionic.BZip2
{
	internal class BZip2Compressor
	{
		 ...
		 * Possibly because the number of elems to sort is usually small, typically less than 20.
		 */
		...
		/*
		 * The following three vars are used when sorting. If too many long comparisons happen, we stop sorting, randomise the block slightly, and
		 * try again. I think this wrinkle in the implementation was removed from a later rev of the C-language bzip, not sure. -DPC 24 Jul 2011
		 */
		...
		public BZip2Compressor( BitWriter writer, int blockSize )
		{
			...
			// 20 provides a margin of slop (not to say "Safety"). The maximum size of an encoded run in the output block is 5 bytes, so really, 5
			// bytes ought to do, but this is a margin of slop found in the original bzip code. Not sure if important for decoding
		...
		private bool AddRunToOutputBlock( bool final )
		{
			...
			if( previousLast >= outBlockFillThreshold && !final )
			{
				string ExceptionMessage = "block overrun(final=" + final + "): " + previousLast + " >= threshold (" + outBlockFillThreshold + ")";
		...
		public void CompressAndWrite() // endBlock
		{
			...
			/*
			 * A 6-byte block header, the value chosen arbitrarily as 0x314159265359 :-). A 32 bit value does not really give a strong enough guarantee
			 * that the value will not appear by chance in the compressed datastream. Worst-case probability of this event, for a 900k block,
			 * is about 2.0e-3 for 32 bits, 1.0e-5 for 40 bits and 4.0e-8 for 48 bits. For a compressed file of size 100Gb -- about 100000 blocks --
			 * only a 48-bit marker will do. NB: normal compression/ decompression do not rely on these statistical properties. They are only important
		...
		{
			CompressionState dataShadow = cstate;
			...
			bool firstAttemptShadow = firstAttempt;
		...
		{
			byte[][] len = cstate.sendMTFValues_len;
		...
		private static void hbMakeCodeLengths( byte[] len, int[] freq, CompressionState state1, int alphaSize, int maxLen )
		{
			...
			for( bool tooLong = true; tooLong; )
			{
				...
				while( nHeap > 1 )
				{
					...
					weight[nNodes] = ( int )( ( ( uint )weight_n1 & 0xffffff00U ) + ( ( uint )weight_n2 & 0xffffff00U ) )
		...
		private int sendMTFValues1( int nGroups, int alphaSize )
		{
			...
			for( int iter = 0; iter < BZip2.N_ITERS; iter++ )
			{
				...
				{
					hbMakeCodeLengths( len[t], rfreq[t], cstate, alphaSize, 20 );
				}
			}
		...
		{
			int[][] code = cstate.sendMTFValues_code;
			byte[][] len = cstate.sendMTFValues_len;
		...
		{
			bool[] inUse = cstate.inUse;
			bool[] inUse16 = cstate.sentMTFValues4_inUse16;
		...
		private void sendMTFValues7( int nSelectors )
		{
			...
			for( int gs = 0; gs < nMTFShadow; )
			{
				...
				while( gs <= ge )
				{
					...
					bw.WriteBits( n, ( uint )code_selCtr[sfmap_i] );
					gs++;
				}
		...
		{
			// with blockSize 900k
			public readonly byte[] block; // 900021 byte
			public readonly int[] fmap; // 3600000 byte
			public readonly int[] ftab = new int[65537]; // 262148 byte
			public readonly byte[] generateMTFValues_yy = new byte[256]; // 256 byte
			public readonly int[] heap = new int[BZip2.MaxAlphaSize + 2]; // 1040 byte
			...
			public readonly int[] parent = new int[BZip2.MaxAlphaSize * 2]; // 2064 byte
			public readonly char[] quadrant;
			...
			public readonly int[][] sendMTFValues_code;
			...
			public readonly byte[][] sendMTFValues_len;

			// byte
			public readonly int[][] sendMTFValues_rfreq;

			// byte
			public readonly bool[] sentMTFValues4_inUse16 = new bool[16]; // 16 byte
			public readonly char[] sfmap; // 3600000 byte
			...
			public readonly int[] weight = new int[BZip2.MaxAlphaSize * 2]; // 2064 byte
