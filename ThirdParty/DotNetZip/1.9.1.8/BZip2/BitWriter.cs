...
namespace Ionic.BZip2
{
	internal class BitWriter
	{
		...
		private int AccumulatedBitCount;
		private uint Accumulator;
		private int TotalBytesWritten;

		public BitWriter( Stream InOutputStream )
		{
		...
		public byte RemainingBits
		{
			get
			{
				return ( byte )( Accumulator >> ( 32 - AccumulatedBitCount ) & 0xff );
			}
		}
		...
		///     </para>
		/// </remarks>
		public void WriteBits( int BitCount, uint OutputValue )
		{
			int Accumulated = AccumulatedBitCount;
			uint CurrentAcumulator = Accumulator;

			while( Accumulated >= 8 )
			{
				OutputStream.WriteByte( ( byte )( CurrentAcumulator >> 24 & 0xff ) );
				TotalBytesWritten++;
				CurrentAcumulator <<= 8;
				Accumulated -= 8;
			}

			Accumulator = CurrentAcumulator | ( OutputValue << ( 32 - Accumulated - BitCount ) );
			AccumulatedBitCount = Accumulated + BitCount;

			// At this point the accumulator may contain up to 31 bits waiting for output.
		}

		/// <summary>
		///     Write a full 8-bit byte into the output.
		/// </summary>
		public void WriteByte( byte OutputByte )
		{
			WriteBits( 8, OutputByte );
		}

		/// <summary>
		///     Write four 8-bit bytes into the output.
		/// </summary>
		public void WriteInt( uint OutputValue )
		{
			WriteBits( 8, ( OutputValue >> 24 ) & 0xff );
			WriteBits( 8, ( OutputValue >> 16 ) & 0xff );
			WriteBits( 8, ( OutputValue >> 8 ) & 0xff );
			WriteBits( 8, OutputValue & 0xff );
		}
		...
		public void FinishAndPad()
		{
			...
			{
				byte OutputByte = ( byte )( ( Accumulator >> 24 ) & 0xff );
