...
namespace Ionic.Zip
{
	...
	internal class WinZipAesCrypto
	{
		...
		private void _GenerateCryptoBytes()
		{
			...
			Rfc2898DeriveBytes rfc2898 = new Rfc2898DeriveBytes( _Password, Salt, Rfc2898KeygenIterations );
	...
	internal class WinZipAesCipherStream : Stream
	{
		...
		private int ReadTransformOneBlock( byte[] buffer, int offset, int last )
		{
			...
			else
			{
				...
				_xform.TransformBlock( counter, 0, // offset
					BLOCK_SIZE_IN_BYTES, counterOut, 0 ); // offset
			}
		...
		public byte[] FinalAuthentication
		{
			get
			{
				...
				Array.Copy( _mac.Hash, 0, macBytes10, 0, 10 );
				return macBytes10;
			}
		}
