...
namespace Ionic.Zip
{
	partial class ZipEntry
	{
		...
		internal bool AttributesIndicateDirectory
		{
			get
			{
				return ( ( _InternalFileAttrs == 0 ) && ( ( _ExternalFileAttrs & 0x0010 ) == 0x0010 ) );
			}
		}
		...
		internal static ZipEntry ReadDirEntry( ZipFile zf, Dictionary<string, Object> previouslySeen )
		{
			...
			if( IsNotValidZipDirEntrySig( signature ) )
			{
				...
				if( signature != ZipConstants.EndOfCentralDirectorySignature && signature != ZipConstants.Zip64EndOfCentralDirectoryRecordSignature && signature != ZipConstants.ZipEntrySignature // workitem 8299
					)
				{
			...
			{
				// UTF-8 is in use
				zde._FileNameInArchive = Encoding.UTF8.GetString( block );
			}
			else
			{
				zde._FileNameInArchive = expectedEncoding.GetString( block, 0, block.Length );
			}
			...
			{
				zde._CompressedFileDataSize = zde.CompressedSize - ( GetLengthOfCryptoHeaderBytes( zde.Encryption ) + 10 );
			...
			if( zde._commentLength > 0 )
			{
				...
				{
					// UTF-8 is in use
					zde._Comment = Encoding.UTF8.GetString( block );
				}
				else
				{
					zde._Comment = expectedEncoding.GetString( block, 0, block.Length );
				}
			}
