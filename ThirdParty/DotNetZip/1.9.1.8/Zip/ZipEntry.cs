	...
	public partial class ZipEntry
	{
		...
		public FileAttributes Attributes
		{
			...
			set
			{
				...
				// "version made by" OS should be zero for compat with WinZip
		...
		public string FileName
		{
			...
			set
			{
				...
				// when renaming, must remove old and then add a new entry
		...
		///    CloseDelegate)" /> and specify
		///         delegates that open and close the stream at
		///         the time of Save.
		///     </para>
		///     <para>
		...
		public CompressionLevel CompressionLevel
		{
			...
			set
			{
				...
				if( value == CompressionLevel.Default && _CompressionMethod == ( short )CompressionMethod.Deflate )
				{
					return; // nothing to do
				}
				_CompressionLevel = value;

				if( value == CompressionLevel.None && _CompressionMethod == ( short )CompressionMethod.None )
				{
		...
		private void SetFdpLoh()
		{
			...
			{
				string description = String.Format( "Exception seeking  entry({0}) offset(0x{1:X8}) len(0x{2:X8})", FileName, _RelativeOffsetOfLocalHeader, ArchiveStream.Length );
			...
			_LengthOfHeader = 30 + extraFieldLength + filenameLength + GetLengthOfCryptoHeaderBytes( _Encryption_FromZipFile );
