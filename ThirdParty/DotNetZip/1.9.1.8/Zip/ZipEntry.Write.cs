...
namespace Ionic.Zip
{
	public partial class ZipEntry
	{
		internal void WriteCentralDirectoryEntry( Stream s )
		{
			...
			// Version Needed To Extract in central directory must be
			...
			// Disk number start
			bool segmented = ( _container.ZipFile != null ) && ( _container.ZipFile.MaxOutputSegmentSize != 0 );
			if( segmented )
			{
		...
		private byte[] ConstructExtraField( bool forCentralDirectory )
		{
			...
			if( _container.Zip64 == Zip64Option.Always || ( _container.Zip64 == Zip64Option.AsNecessary && ( !forCentralDirectory || _entryRequiresZip64.Value ) ) )
			{
				...
				// only include this if the "extra" field is for
		...
		private byte[] GetEncodedFileNameBytes()
		{
			...
			// when writing, use the alternative encoding
		...
		private void MaybeUnsetCompressionMethodForWriting( int cycle )
		{
			...
			if( CompressionLevel == ( short )CompressionLevel.None && CompressionMethod == CompressionMethod.Deflate )
			{
				_CompressionMethod = 0x00;
			}
		}
		...
		internal void WriteHeader( Stream s, int cycle )
		{
			...
			// WinZip does not the "strong encryption" bit  when using AES.
			...
			//
			// having trouble formatting a zip64 file that is
		...
		private Int32 FigureCrc32()
		{
			if( _crcCalculated == false )
			{
				...
				else
				{
					...
					_Crc32 = crc32.GetCrc32AndCopy( input, null );
		...
		private long SetInputAndFigureFileLength( ref Stream input )
		{
			...
			else if( _Source == ZipEntrySource.ZipFile )
			{
				...
				string pwd = ( _Encryption_FromZipFile == EncryptionAlgorithm.None ) ? null : ( _Password ?? _container.Password );
		...
		internal void PostProcessOutput( Stream s )
		{
			...
			// sometimes we don't seek even if we CAN.  ASP.NET
			...
			if( ( _BitField & 0x0008 ) != 0x0008 || ( _Source == ZipEntrySource.ZipOutputStream && s.CanSeek ) )
			{
		...
		private Stream MaybeApplyCompression( Stream s, long streamLength )
		{
			if( _CompressionMethod == 0x08 && CompressionLevel != CompressionLevel.None )
			{
				...
				DeflateStream o = new DeflateStream( s, CompressionMode.Compress, CompressionLevel, true );
				if( _container.CodecBufferSize > 0 )
				{
			...
			if( _CompressionMethod == 0x0c )
			{
				if( _container.ParallelDeflateThreshold == 0L || ( streamLength > _container.ParallelDeflateThreshold && _container.ParallelDeflateThreshold > 0L ) )
				{
					ParallelBZip2OutputStream o1 = new ParallelBZip2OutputStream( s, true );
					return o1;
				}
				BZip2OutputStream o = new BZip2OutputStream( s, true );
				return o;
			}
#endif
		...
		private void CopyThroughOneEntry( Stream outStream )
		{
			...
			bool needRecompute = _metadataChanged || ( ArchiveStream is ZipSegmentedStream ) || ( outStream is ZipSegmentedStream ) || ( _InputUsesZip64 && _container.UseZip64WhenSaving == Zip64Option.Never )
		...
		private void CopyThroughWithRecompute( Stream outstream )
		{
			...
			CountingStream input = new CountingStream( ArchiveStream );
