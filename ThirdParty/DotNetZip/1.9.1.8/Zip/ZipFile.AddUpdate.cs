...
namespace Ionic.Zip
{
	public partial class ZipFile
	{
		...
		/// </param>
		/// <param name="encoding">
		///     The text encoding to use when encoding the string. 
		/// </param>
		/// <returns>The <c>ZipEntry</c> added.</returns>
		...
		public ZipEntry AddDirectoryByName( string directoryNameInArchive )
		{
			...
			dir.MarkAsDirectory();
			dir.AlternateEncoding = AlternateEncoding;
		...
		private ZipEntry AddOrUpdateDirectoryImpl( string directoryName, string rootDirectoryPathInArchive, AddOrUpdateAction action, bool recurse, int level )
		{
			...
			if( level > 0 || !String.IsNullOrEmpty( rootDirectoryPathInArchive ) )
			{
				baseDir = ZipEntry.CreateFromFile( directoryName, dirForEntries );
				baseDir._container = new ZipContainer( this );
				baseDir.AlternateEncoding = AlternateEncoding;
			...
			if( !_addOperationCanceled )
			{
				...
				if( recurse )
				{
					...
					if( !_addOperationCanceled )
					{
						...
						{
							// Optionally traverse reparse points
