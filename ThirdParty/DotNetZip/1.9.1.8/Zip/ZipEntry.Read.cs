	...
	public partial class ZipEntry
	{
		...
		private static bool ReadHeader( ZipEntry ze, Encoding defaultEncoding )
		{
			...
			// the three amigos may contain marker values for ZIP64.  So we must read them.
			{
				...
				if( ( uint )ze._CompressedSize == 0xFFFFFFFF || ( uint )ze._UncompressedSize == 0xFFFFFFFF )
				{
		...
		private int ProcessExtraFieldWinZipAes( byte[] buffer, int j, Int16 dataSize, long posn )
		{
			if( _CompressionMethod == 0x0063 )
			{
				...
				_Encryption_FromZipFile = _Encryption = ( keystrength == 128 ) ? EncryptionAlgorithm.WinZipAes128 : EncryptionAlgorithm.WinZipAes256;
		...
		private int ProcessExtraFieldUnixTimes( byte[] buffer, int j, Int16 dataSize, long posn )
		{
			...
			if( dataSize == 13 || _readExtraDepth > 0 )
			{
				...
				_Atime = ( ( flag & 0x0002 ) != 0 && remainingData >= 4 ) ? slurp() : DateTime.UtcNow;

				_Ctime = ( ( flag & 0x0004 ) != 0 && remainingData >= 4 ) ? slurp() : DateTime.UtcNow;
