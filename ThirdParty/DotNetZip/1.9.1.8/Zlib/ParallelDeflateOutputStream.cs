		...
		: Stream
	{
		...
		public override void Write( byte[] buffer, int offset, int count )
		{
			...
			do
			{
				...
				{
					// No need for interlocked.increment: the Write() method is documented as not multi-thread safe, so
		...
		{
			// After writing a series of compressed buffers, each one closed with Flush.Sync, we now write the final one as Flush.Finish, and then stop.
		...
		{
			// When combining parallel deflation with a ZipSegmentedStream, it's possible for the ZSS to throw from within this method.  In that
			// case, Close/Dispose will be called on this stream, if this stream is employed within a using or try/finally pair as required. But
			// this stream is unaware of the pending exception, so the Close() method invokes this method AGAIN.  This can lead to a deadlock.
		...
		private void _DeflateOne( Object wi )
		{
			...
			try
			{
				...
				crc.ChecksumBlock( workitem.buffer, 0, workitem.inputBytesAvailable );
