			...
			for (int i = 0; i < 2; i++) // run once for ascii, once for unicode
			{
				...
				try
				{
					...
					using (Connection con = rep.Connection)
					{
						...
						//IPHostEntry ipEntry = System.Net.Dns.GetHostEntry(machinename);
						...
						Assert.AreEqual(machinename, c.HostName, true);

						//Assert.AreEqual(addr, c.Address);

						Assert.AreEqual("C:\\MyTestDir\\admin_space", c.CurrentDirectory, true);
