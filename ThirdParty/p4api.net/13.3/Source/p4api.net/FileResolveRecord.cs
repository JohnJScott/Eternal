...
namespace Perforce.P4
{
	...
	public class ResolveAnalysis
	{
		public ResolveAnalysis()
		{
			...
			//SuggestedAction = P4ClientMerge.MergeStatus.CMS_NONE;
		}
	...
	public class FileResolveRecord
	{
		...
		public static FileResolveRecord FromMergeInfo(P4.InfoList info)
		{
			if (info.Count <= 0)
			{
				return null;
			...
			if (info[0].Info.Contains("Diff chunks:"))
			{
				l2 = info[0].Info;
				if (info.Count >= 2)
				{
			...
			else if ((info.Count >= 2) && (info[1].Info.Contains("Diff chunks:")))
			{
				l1 = info[0].Info;
				l2 = info[1].Info;
				if (info.Count >= 2)
			...
			{
				if (info[0].Info.Contains(" - resolve "))
				{
					l3 = info[0].Info;
				}
				else
