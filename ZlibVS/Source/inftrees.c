...
{
    uint32 len;               /* a code's length in bits */
    uint32 sym;               /* index of code symbols */
    uint32 min, max;          /* minimum and maximum code lengths */
    uint32 root;              /* number of index bits for root table */
    uint32 curr;              /* number of index bits for current table */
    uint32 drop;              /* code bits to drop for sub-table */
    int left;                   /* number of prefix codes available */
    uint32 used;              /* code entries in table used */
    uint32 huff;              /* Huffman code */
    uint32 incr;              /* for incrementing code, index */
    uint32 fill;              /* index for replicating entries */
    uint32 low;               /* low bits for current root entry */
    uint32 mask;              /* mask for low root bits */
    ...
    {   /* no symbols to code at all */
        here.op = ( uint8 )64;    /* invalid code marker */
        here.bits = ( uint8 )1;
        here.val = ( uint16 )0;
    ...
    /*
       ...
       the constants ENOUGH_LENS and ENOUGH_DISTS to guard against changes in the initial root table size constants.  See the comments in inftrees.h
       for more information.

       sym increments through all symbols, and the loop terminates when all codes of length max, i.e. all codes, have been processed.  This
       routine permits incomplete codes, so another loop after this one fills in the rest of the decoding tables with invalid code markers.
     */
    ...
    low = ( uint32 )( -1 );       /* trigger new sub-table when len > root */
    ...
    /* fill in remaining table entry if code is incomplete (guaranteed to have at most one remaining entry, since if the code is incomplete, the
