...
typedef struct inflate_state_s
{
    ...
    int32 last;					/* true if processing last block */
    int32 wrap;					/* bit 0 true for zlib, bit 1 true for gzip */
    int32 havedict;				/* true if dictionary provided */
    int32 flags;				/* gzip header method and flags (0 if zlib) */
    uint32 dmax;				/* zlib header max distance (INFLATE_STRICT) */
    ...
    int32 sane;					/* if false, allow invalid distance too far */
    int32 back;					/* bits back of last unprocessed length/lit */
    uint32 was;					/* initial length of match */
} inflate_state;
